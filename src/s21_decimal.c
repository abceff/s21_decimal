...
s21_decimal s21_add(s21_decimal number_1, s21_decimal number_2) {
    ...
    if (res.value_type == s21_NORMAL_VALUE || res.value_type == s21_ADDCODE) {
        if (!get_sign(&number_1) && !get_sign(&number_2)) {
            ...
            if (tmpRes.value_type == s21_INFINITY &&
                ...
                while (res.value_type == s21_INFINITY &&
                    ...
                    s21_decimal tmpDiv1 =
                        div_only_bits(number_1, ten, &remainder1);
                    s21_decimal tmpDiv2 =
                        div_only_bits(number_2, ten, &remainder2);
...
int check_for_mul(s21_decimal number_1, s21_decimal number_2) {
    ...
    //     res.value_type = s21_NAN;
    // } else 
    if ((there_is_plus_normal && there_is_inf) ||
...
int s21_mul(s21_decimal number_1, s21_decimal number_2, s21_decimal* result) {
    ...
    // Это мы можем убрать, так как тут просто отсекаются некорректные входные значения
    ...
    while (bit_addition_result != OK &&
        ...
        s21_decimal remainder;
        s21_decimal tmpDiv =
            div_only_bits(*chosen_numberPtr, ten, &remainder);
